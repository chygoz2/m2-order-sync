...
class UpdateOrderStatusInCore {
    ...
    public function execute()
    {
        try {
            ...
            $this->logger->info("All order available have been sync with core");
    ...
    public function syncWithCore($order){
        $order['row']['magento_core_secret'] = 'secret';
        $params = $order['row'];
        $incrementId = $order['row']['increment_id'];

        try {
            $this->httpClient->reset();
            $this->httpClient->setUri('http://192.168.99.100:1337/sync');
            ...
            if($statusCode !== 201){
                $this->logger->critical("Order with increment id {$incrementId} failed to sync with core, an attempt will be made in the next sync === $statusCode");
            } else {
                $orderFactory = $this->orderFactory->create();
                $order = $orderFactory->load($incrementId, 'increment_id');
                $order->delete();
        
                $this->logger->info("sync with core {$data}");
            }
